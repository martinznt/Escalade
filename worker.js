...
async function login(request, env, secure) {
  ...
  if (await rlBlocked(env, rk, 10, 900000)) return fail('Trop d’essais. Réessaie dans quelques minutes.', 429);
  ...
  if (!row || !safeEq(computed, row.password_hash)) { await rlHit(env, rk, 900000); return fail('Pseudo ou mot de passe incorrect.', 401); }
