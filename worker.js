...
async function settingsGet(u,e){const x=await e.DB.prepare('SELECT settings_json FROM user_data WHERE user_id=?').bind(u.id).first();return json(JSON.parse(x?.settings_json||'{}'))}
async function settingsPost(r,u,e){const b=await body(r);if(!b||typeof b!=='object')return json({ok:false,error:'Format invalide.'},400);await e.DB.prepare('UPDATE user_data SET settings_json=?,updated_at=? WHERE user_id=?').bind(JSON.stringify(b),Date.now(),u.id).run();return json({ok:true})}
async function calendarGet(r,u,e){const q=new URL(r.url).searchParams,from=q.get('from'),to=q.get('to');let sql='SELECT id,event_date,session_id,title,completed,recurrence_json FROM calendar_events WHERE user_id=?',a=[u.id];if(from){sql+=' AND event_date>=?';a.push(from)}if(to){sql+=' AND event_date<=?';a.push(to)}sql+=' ORDER BY event_date,id';const x=await e.DB.prepare(sql).bind(...a).all();return json(x.results.map(v=>({...v,completed:!!v.completed,recurrence:v.recurrence_json?JSON.parse(v.recurrence_json):null})))} 
async function calendarPost(r,u,e){const b=await body(r);if(!b||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(b.date))return json({ok:false,error:'Date invalide.'},400);const now=Date.now(),eid=b.id||id();const existing=await e.DB.prepare('SELECT id FROM calendar_events WHERE id=? AND user_id=?').bind(eid,u.id).first();if(existing){await e.DB.prepare('UPDATE calendar_events SET event_date=?,session_id=?,title=?,completed=?,recurrence_json=?,updated_at=? WHERE id=? AND user_id=?').bind(b.date,b.sessionId||null,b.title||null,b.completed?1:0,b.recurrence?JSON.stringify(b.recurrence):null,now,eid,u.id).run()}else{await e.DB.prepare('INSERT INTO calendar_events(id,user_id,event_date,session_id,title,completed,recurrence_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(eid,u.id,b.date,b.sessionId||null,b.title||null,b.completed?1:0,b.recurrence?JSON.stringify(b.recurrence):null,now,now).run()}return json({ok:true,id:eid})}
...
async function publicProfile(p,e){const username=decodeURIComponent(p.split('/').pop());const row=await e.DB.prepare('SELECT u.username,p.display_name,p.bio,p.public_profile,p.share_progress,p.share_workouts,p.avatar_emoji FROM users u JOIN profiles p ON p.user_id=u.id WHERE lower(u.username)=lower(?)').bind(username).first();if(!row||!row.public_profile)return json({ok:false,error:'Profil privé ou introuvable.'},404);let metrics=[];if(row.share_progress){const x=await e.DB.prepare('SELECT metric,value,unit,recorded_at FROM metrics WHERE user_id=? ORDER BY recorded_at DESC LIMIT 100').bind(row.user_id).all();metrics=x.results}return json({profile:row,metrics})}
